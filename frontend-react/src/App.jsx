  ...
  return (
    <div className="min-h-screen bg-[var(--color-bg-primary)] text-[var(--color-text-primary)] flex flex-col items-center pt-12 pb-8 px-4 sm:px-6 lg:px-8" style={{ backgroundImage: 'var(--gradient-hero)' }}>
      ...
      <header className="text-center mb-10 md:mb-12 max-w-3xl w-full">
        ...
        {/* Contact Links */}
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4 max-w-sm sm:max-w-none mx-auto w-full sm:w-auto flex-wrap">
          <a
      ...
      {/* Footer Section */}
      <footer className="w-full max-w-5xl mt-16 pt-8 pb-4 border-t border-[var(--color-border-primary)] flex flex-col md:flex-row items-center justify-between gap-4 text-xs text-[var(--color-text-muted)]">
        <div className="flex items-center space-x-2">
          <span>&copy; {new Date().getFullYear()} {portfolioConfig.name}. All rights reserved.</span>
        </div>
        <div className="flex items-center space-x-1 text-[var(--color-text-faint)]">
          <span>Architected with</span>
          <span className="text-[var(--color-accent-orange)] font-semibold font-mono">Rust</span>
          <span>&bull;</span>
          <span className="text-[var(--color-accent-emerald)] font-semibold font-mono">Spring Boot</span>
          <span>&bull;</span>
          <span className="text-[var(--color-accent-teal)] font-semibold font-mono">React</span>
        </div>
        <div className="flex items-center space-x-4">
          <a href={portfolioConfig.githubUrl} target="_blank" rel="noopener noreferrer" className="hover:text-[var(--color-hover-text)] transition-colors duration-200" title="GitHub">
            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              <path fillRule="evenodd" clipRule="evenodd" d="M12 2C6.477 2 2 6.477 2 12c0 4.42 2.865 8.166 6.839 9.489.5.092.682-.217.682-.482 0-.237-.008-.866-.013-1.7-2.782.603-3.369-1.34-3.369-1.34-.454-1.156-1.11-1.464-1.11-1.464-.908-.62.069-.608.069-.608 1.003.07 1.531 1.03 1.531 1.03.892 1.529 2.341 1.087 2.91.831.092-.646.35-1.086.636-1.336-2.22-.253-4.555-1.11-4.555-4.943 0-1.091.39-1.984 1.029-2.683-.103-.253-.446-1.27.098-2.647 0 0 .84-.269 2.75 1.025A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.294 2.747-1.025 2.747-1.025.546 1.377.203 2.394.1 2.647.64.699 1.028 1.592 1.028 2.683 0 3.842-2.339 4.687-4.566 4.935.359.309.678.919.678 1.852 0 1.336-.012 2.415-.012 2.743 0 .267.18.579.688.481C19.138 20.161 22 16.418 22 12c0-5.523-4.477-10-10-10z" />
            </svg>
          </a>
          <a href={portfolioConfig.linkedinUrl} target="_blank" rel="noopener noreferrer" className="hover:text-[var(--color-hover-text)] transition-colors duration-200" title="LinkedIn">
            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
            </svg>
          </a>
          <a href={`mailto:${portfolioConfig.email}`} className="hover:text-[var(--color-hover-text)] transition-colors duration-200" title="Email">
      ...
      {showDevOpsCaseStudy && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[var(--color-bg-overlay)] backdrop-blur-md transition-opacity duration-300 overflow-y-auto">
          <div className="relative bg-[var(--color-bg-modal)] border border-[var(--color-border-secondary)] rounded-2xl w-full max-w-3xl p-6 md:p-8 shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto custom-scrollbar">
            ...
            <div className="flex justify-between items-start border-b border-[var(--color-border-primary)] pb-4 mb-6">
              ...
              <button
                ...
                className="text-[var(--color-text-tertiary)] hover:text-[var(--color-hover-text)] p-1 hover:bg-[var(--color-hover-bg)] rounded-lg transition-colors cursor-pointer"
              >
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 items-stretch justify-center my-4 py-4 px-3 border border-[var(--color-border-primary)] bg-[var(--color-hover-bg)] rounded-xl">
                  {/* Client */}
                  <div className="flex flex-col items-center justify-between bg-[var(--color-bg-surface-elevated)] p-3 rounded-lg border border-[var(--color-border-primary)] text-center shadow-md">
                    <span className="text-xl">🌐</span>
                    <span className="text-[10px] font-bold text-[var(--color-text-primary)] uppercase tracking-widest mt-1">Client Dashboard</span>
                    <span className="text-[9px] text-[var(--color-text-muted)] mt-1">Vite + React UI Client</span>
                  </div>
                  {/* Spring Gateway */}
            ...
            <div className="mt-8 pt-4 border-t border-[var(--color-border-primary)] flex flex-col sm:flex-row gap-3 justify-end items-center">
              <a
                href="https://devops.mattdev0.tech"
                target="_blank"
                rel="noopener noreferrer"
                className="w-full sm:w-auto px-5 py-2.5 bg-[var(--color-accent-emerald-solid)] hover:bg-[var(--color-accent-emerald-solid-hover)] text-white font-bold rounded-xl text-xs text-center transition-colors cursor-pointer flex items-center justify-center space-x-1.5"
                style={{ boxShadow: 'var(--shadow-button)' }}
              >
                <span>🚀</span>
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="w-full sm:w-auto px-5 py-2.5 bg-[var(--color-hover-bg)] border border-[var(--color-border-primary)] hover:bg-[var(--color-hover-bg)] hover:border-[var(--color-border-secondary)] text-[var(--color-text-secondary)] hover:text-[var(--color-hover-text)] font-semibold rounded-xl text-xs text-center transition-colors cursor-pointer"
              >
                View Repository Code
              </a>
              <button
                onClick={() => setShowDevOpsCaseStudy(false)}
                className="w-full sm:w-auto px-5 py-2.5 bg-[var(--color-bg-tag)] hover:bg-[var(--color-hover-bg)] text-[var(--color-text-secondary)] hover:text-[var(--color-hover-text)] font-semibold rounded-xl text-xs cursor-pointer"
              >
                Close Case Study
              </button>
            </div>
